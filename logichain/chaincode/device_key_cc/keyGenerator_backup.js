...
function main() {
    ...
    const testTx = generateTestTx(keyStore[0].priKey,message);
    const signature = JSON.parse(testTx).signature;
    
    console.log(testTx);
    console.log(`Verify with correct publickey: ${doVerify(keyStore[0].pubKey,signature,JSON.stringify(message))}`);
    console.log(`Verify with wrong publickey: ${doVerify(keyStore[1].pubKey,signature,JSON.stringify(message))}`);
    console.log(`Verify with wrong data: ${doVerify(keyStore[0].pubKey,signature,JSON.stringify(wrongMessage))}`);
